...
public class PACEProtocol {
  ...
  private PACEResult doPACE(AccessKeySpec accessKey, SecretKey staticPACEKey, String oid, AlgorithmParameterSpec staticParameters, BigInteger parameterId) throws CardServiceException {
     ...
     */
    byte[] piccNonce = doPACEStep1(staticPACEKey, staticPACECipher);//5
    ...
    PACEMappingResult mappingResult = doPACEStep2(mappingType, agreementAlg, staticParameters, piccNonce, staticPACECipher);//6
     ...
     */
    PublicKey ephemeralPICCPublicKey = doPACEStep3ExchangePublicKeys(ephemeralPCDKeyPair.getPublic(), ephemeralParams);//7
     ...
     */
    byte[] encryptedChipAuthenticationData = doPACEStep4(oid, mappingType, ephemeralPCDKeyPair, ephemeralPICCPublicKey, macKey);//8
  ...
  public static ECParameterSpec mapNonceGMWithECDH(byte[] nonceS, ECPoint sharedSecretPointH, ECParameterSpec staticParameters) {
    ...
    ECPoint multiplied = Util.multiply(Util.os2i(nonceS), generator, staticParameters);
    ECPoint ephemeralGenerator = Util.add(multiplied, sharedSecretPointH, staticParameters);
